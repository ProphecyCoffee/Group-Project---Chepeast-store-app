...
class CalculateData : public DataExtractor<T>
{
	...
	*/
    ...
    double Mean(int item)
    {
        ...
        {
            for (auto s : item.Stores)
            {
                int mean = mean + s.price;
                count++;
            }
        }
        mean = mean / count;
	...
	*/
    ...
    //Andrew Majewski
    // Bubble sort prices where stores will be ordered at the cheapest at [0]
    void BubbleSortPrices()
    {
    ...
    void findBestStore() // Wrote as a setter.
    {
        ...
        string bestStoreName = Items.Stores.;
    ...
    */
    ...
    // Destin Mingledoff, Move to Display
...
// Imported from calculate Display:
    ...
    void findBestPrice() // wrote as a setter. 
    {
        ...
        for (auto& an_Item : items) {
            ...
            // TO DO: Send string to log function to have it sent to the log file. 
